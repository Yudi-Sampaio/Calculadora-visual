...
def listar_historico():
    ...
    for i, h in enumerate(historico, start=1):
        texto += f"{i}.{h['expressao']} = {h['resultado']}\n"
...
def atualizar_calculo():
    ...
    if 0 <= indice < len(historico):
        ...
        try:
            ...
            historico[indice]["expressao"] = nova_expr
            historico[indice]["resultado"] = novo_result
...
def deletar_calculo():
    ...
    if 0 <= indice < len(historico):
        removido = historico.pop(indice)
        messagebox.showinfo("Deletar", f"cálculo{removido['expressao']} deletado!")
    else:
